...
public static class DetourHelper
{
    ...
    {
        get => DetourRuntimeReorgPlatform.Instance;
    ...
    {
        MethodInfo dm;
        using (var dmd = new MethodSignature(signature, true).CreateDmd($"Native<{((long) target).ToString("X16", CultureInfo.InvariantCulture)}>"))
        {
            dm = dmd.StubCriticalDetour().Generate().Pin();
        }

        var detour = DetourFactory.Current.CreateNativeDetour(dm.GetNativeStart(), target);
        GC.SuppressFinalize(detour);
